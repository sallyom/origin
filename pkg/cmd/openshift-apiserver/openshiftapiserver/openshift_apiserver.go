...
func (c *completedConfig) bootstrapSCC(context genericapiserver.PostStartHookContext) error {
	...
	jsonKubeAPIClientConfig := restclient.CopyConfig(c.ExtraConfig.KubeAPIServerClientConfig)
	jsonKubeAPIClientConfig.ContentConfig.AcceptContentTypes = "application/json"
	jsonKubeAPIClientConfig.ContentConfig.ContentType = "application/json"
	err := wait.Poll(1*time.Second, 30*time.Second, func() (bool, error) {
		var err error
		securityClient, err = securityv1client.NewForConfig(jsonKubeAPIClientConfig)
		if err != nil {
