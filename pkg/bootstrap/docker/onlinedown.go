...
`
	...
	CmdOnlineDownRecommendedName = "onlinedown"
)

type ClientOnlineStopConfig struct {
	DockerMachine string
}

// NewCmdDown creates a command that stops OpenShift
func NewCmdOnlineDown(name, fullName string, f *osclientcmd.Factory, out io.Writer) *cobra.Command {
	onlineconfig := &ClientOnlineStopConfig{}
	cmd := &cobra.Command{
		Use:     name,
		Short:   "Stop OpenShift on Docker",
		Long:    cmdOnlineDownLong,
		Example: fmt.Sprintf(cmdOnlineDownExample, fullName),
		Run: func(c *cobra.Command, args []string) {
			kcmdutil.CheckErr(onlineconfig.Stop(out))
		},
	}
...
func (c *ClientOnlineStopConfig) Stop(out io.Writer) error {
...
func (c *ClientOnlineStopConfig) getDockerClient(out io.Writer) (*docker.Client, error) {
	// Get Docker client
